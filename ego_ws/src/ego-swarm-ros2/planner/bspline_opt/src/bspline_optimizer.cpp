...
namespace ego_planner
{
        ...
        for (int i = 0; i < seg_upbound; i++)
        {
            ...
            if (RichInfoSegs[i].first.size > 1)
            {
                ...
                for (int j = 0; j < RichInfoSegs[i].first.size - 1; j++)
                {
                    ...
                    double step_size = RESOLUTION / (RichInfoSegs[i].first.points.col(j) - RichInfoSegs[i].first.points.
                        col(j + 1)).norm() / 2;
                    for (double a = 1; a > 0; a -= step_size)
                    {
                        Eigen::Vector3d pt(
                            a * RichInfoSegs[i].first.points.col(j) + (1 - a) * RichInfoSegs[i].first.points.col(
                                j + 1));
            ...
            exit_multi_loop2:;
                ...
                for (int j = occ_start_id; j <= occ_end_id; j++)
                {
                    ...
                    {
                        cout << "RichInfoSegs[" << i << "].first.base_point[" << j << "].size()=" << RichInfoSegs[i].
                            first.base_point[j].size() << endl;
                        ...
                        for (int temp_i = 0; temp_i < cps_.size; temp_i++)
                        {
                            if (cps_.base_point[temp_i].size() > 1 && cps_.base_point[temp_i].size() < 1000)
                            {
                                ...
                                for (size_t temp_j = 0; temp_j < cps_.base_point[temp_i].size(); temp_j++)
                                    cout << "      " << cps_.base_point[temp_i][temp_j].transpose() << " @ " << cps_.
                                        direction[temp_i][temp_j].transpose() << endl;
                            }
                        }
                    ...
                    {
                        base_pt_reverse = RichInfoSegs[i].first.points.col(j) + base_vec_reverse * (RichInfoSegs[i].
                            first.base_point[j][0] - RichInfoSegs[i].first.points.col(j)).norm();
                    }
                    ...
                    else
                    {
                        ...
                        cout << "base_point=" << RichInfoSegs[i].first.base_point[j][0].transpose() << " control point="
            ...
            else if (RichInfoSegs[i].first.size == 1)
            {
                ...
                cout << "RichInfoSegs[i].first.direction[0].size()=" << RichInfoSegs[i].first.direction[0].size() <<
                    endl;
                ...
                cout << "RichInfoSegs[i].first.base_point[0].size()=" << RichInfoSegs[i].first.base_point[0].size() <<
                    endl;
                Eigen::Vector3d base_vec_reverse = -RichInfoSegs[i].first.direction[0][0];
                Eigen::Vector3d base_pt_reverse = RichInfoSegs[i].first.points.col(0) + base_vec_reverse * (RichInfoSegs
                    [i].first.base_point[0][0] - RichInfoSegs[i].first.points.col(0)).norm();
    ...
    // 初始化控制点
    std::vector<std::pair<int, int>> BsplineOptimizer::initControlPoints(
        Eigen::MatrixXd& init_points, bool flag_first_init /*= true*/)
    {
        ...
        // 障碍物检测的步长
        double step_size = grid_map_->getResolution() / ((init_points.col(0) - init_points.rightCols(1)).norm() / (
            init_points.cols() - 1)) / 1.5;
        ...
        for (int i = order_; i <= i_end; ++i)
        {
            ...
            for (double a = 1.0; a > 0.0; a -= step_size)
            {
                ...
                // 如果已经离开障碍物，则结束
                if (flag_got_end_maybe && (same_occ_state_times > ENOUGH_INTERVAL || (i == (int)init_points.cols() -
                    order_)))
                {
        ...
        // Each segment is guaranteed to have sufficient points to generate sufficient force
        // 控制点的最小数量
        ...
        for (size_t i = 0; i < segment_ids.size(); i++)
        {
                    ...
                    // val = last_val = 0.0 is not allowed
                    {
                        ...
                        intersection_point =
                            ...
                            ((a_star_pathes[i][Astar_id] - a_star_pathes[i][last_Astar_id]) *
                                (ctrl_pts_law.dot(init_points.col(j) - a_star_pathes[i][Astar_id]) / ctrl_pts_law.dot(
            ...
            if (segment_ids[i].second - segment_ids[i].first == 1)
            {
                ...
                Eigen::Vector3d ctrl_pts_law(
                                    init_points.col(segment_ids[i].second) - init_points.col(segment_ids[i].first)),
                                intersection_point;
                Eigen::Vector3d middle_point = (init_points.col(segment_ids[i].second) + init_points.col(
                    segment_ids[i].first)) / 2;
                // 计算A*中点并判断方向，同上
                int Astar_id = a_star_pathes[i].size() / 2, last_Astar_id;
                // Let "Astar_id = id_of_the_most_far_away_Astar_point" will be better, but it needs more computation
                ...
                while (Astar_id >= 0 && Astar_id < (int)a_star_pathes[i].size())
                {
                    ...
                    // val = last_val = 0.0 is not allowed
                    {
                        ...
                        intersection_point =
                            a_star_pathes[i][Astar_id] +
                            ((a_star_pathes[i][Astar_id] - a_star_pathes[i][last_Astar_id]) *
                                (ctrl_pts_law.dot(middle_point - a_star_pathes[i][Astar_id]) / ctrl_pts_law.dot(
                                    a_star_pathes[i][Astar_id] - a_star_pathes[i][last_Astar_id])) // = t
                            );
        ...
        for (int i = order_; i < end_idx; i++)
        {
            ...
            for (size_t id = 0; id < swarm_trajs_->size(); id++)
            {
                ...
                if (glb_time < traj_i_satrt_time + swarm_trajs_->at(id).duration_ - 0.1)
                {
                    ...
                    double ellip_dist = sqrt(
                        dist_vec(2) * dist_vec(2) * inv_a2 + (dist_vec(0) * dist_vec(0) + dist_vec(1) * dist_vec(1)) *
                        inv_b2);
    ...
    bool BsplineOptimizer::check_collision_and_rebound(void)
    {
        ...
        for (int i = order_ - 1; i <= i_end; ++i)
        {
            ...
            if (occ)
            {
                ...
                {
                    cout.precision(2);
                    if ((cps_.points.col(i) - cps_.base_point[i][k]).dot(cps_.direction[i][k]) < 1 * grid_map_->
                        getResolution()) // current point is outside all the collision_points.
                    {
                    ...
                    while (Astar_id >= 0 && Astar_id < (int)a_star_pathes[i].size())
                    {
                        ...
                        // val = last_val = 0.0 is not allowed
                        {
                            intersection_point =
                                a_star_pathes[i][Astar_id] +
                                ((a_star_pathes[i][Astar_id] - a_star_pathes[i][last_Astar_id]) *
                                    (ctrl_pts_law.dot(cps_.points.col(j) - a_star_pathes[i][Astar_id]) / ctrl_pts_law.
                                        dot(a_star_pathes[i][Astar_id] - a_star_pathes[i][last_Astar_id])) // = t
                                );
    ...
    bool BsplineOptimizer::rebound_optimize(double& final_cost)
    {
        ...
        do
        {
                ...
                result == lbfgs::LBFGS_STOP)
            {
                ...
                // 计算时间步长
                double t_step = (tmp - tm) / ((traj.evaluateDeBoorT(tmp) - traj.evaluateDeBoorT(tm)).norm() / grid_map_
                    ->getResolution());
                // 遍历轨迹的前2/3部分进行障碍物检测
        ...
        if ((flag_cls_xyp ^ flag_cls_xyn) || (flag_cls_zp ^ flag_cls_zn))
          flag_occ = true;
            ...
            {
                RCLCPP_WARN(rclcpp::get_logger("rebound_optimize"),
                            "Solver error. Return = %d, %s. Skip this planning.", result,
    ...
    void BsplineOptimizer::combineCostRebound(const double* x, double* grad, double& f_combine, const int n)
    {
        ...
        f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance + lambda3_ * f_feasibility + new_lambda2_ *
            f_swarm + lambda2_ * f_terminal;
