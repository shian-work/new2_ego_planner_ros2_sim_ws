...
*/
#ifndef LINEAR_OBJ_MODEL_H
#define LINEAR_OBJ_MODEL_H
...
inline void LinearObjModel::initialize(Eigen::Vector3d p, Eigen::Vector3d v, Eigen::Vector3d a, double yaw,
...
inline void LinearObjModel::setLimits(Eigen::Vector3d bound, Eigen::Vector2d vel, Eigen::Vector2d acc)
{
    bound_ = bound;
    limit_v_ = vel;
    limit_a_ = acc;
...
inline void LinearObjModel::update(double dt)
{
    ...
    if (input_type_ == 2)
    {
        ...
        {
            if (vel_(i) > 0)
                vel_(i) = std::max(limit_v_(0), std::min(vel_(i),
                                                         limit_v_(1)));
            if (vel_(i) <= 0)
                vel_(i) = std::max(-limit_v_(1), std::min(vel_(i),
                                                          -limit_v_(0)));
        }
...
inline bool LinearObjModel::collide(LinearObjModel& obj1, LinearObjModel& obj2)
{
